# SensorSpeak - Accelerometer Motion Event Explainer

Your sensor logs 100,000 numbers per minute. SensorSpeak turns those numbers into plain-English answers you can act on.

Runs fully offline. No cloud, no API keys, no ML training needed.
...
SensorSpeak reads accelerometer CSV data (or generates synthetic data if you have none) and does three things:
...
The MEMS accelerometers (BMA400, BMI088, BMG250, etc.) are small chips used in phones, fitness trackers, industrial machines, conveyor belts, and robots. They output a continuous stream of numbers at a fixed rate - typically 100 readings per second.
...
| `pdf_qa.py` | sensor datasheet ingestion - ask questions about sensor specs alongside recorded data |
...
- Live hardware streaming from BMA400/BMI088 via serial (pyserial) or MQTT (paho-mqtt)
...
├── pdf_qa.py                ← sensor datasheet ingestion
...
*Built with Python, LangChain, LangGraph, Ollama, and raw accelerometer data.*
